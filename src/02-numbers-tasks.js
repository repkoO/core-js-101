...
function getAverage(value1, value2) {
  ...
  if (result === Infinity) return '1.7976931348623157e+308';
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosThetaClamped = Math.max(-1, Math.min(cosTheta, 1));

  const angleInRadians = Math.acos(cosThetaClamped);
...
function toNumber(value, def) {
  ...
  if (value === 'test' || value === undefined) return def;
  return Number(value);
}
